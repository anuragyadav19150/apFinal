...
public class rectanglePiece extends Obstacle{

	public rectanglePiece(Group g) {
		super(g);
	...
	public void hitBall(Ball c,Stage stage,Scene s1,Scene s2,gameplay game) {
		...
		else if(((Path)Shape.intersect(c.ball, (Shape) this.getG().getChildren().get(4))).getElements().size()>0) 
		{
			if(!c.getColor().equals(Color.rgb(255,0 , 255).toString())) {
				try {
					a.start11(stage, s1, s2,game,c);
		...
		else if(((Path)Shape.intersect(c.ball, (Shape) this.getG().getChildren().get(5))).getElements().size()>0) 
		{
			if(!c.getColor().equals(Color.rgb(255,0 , 0).toString())) {
				try {
					a.start11(stage, s1, s2,game,c);
		...
		else if(((Path)Shape.intersect(c.ball, (Shape) this.getG().getChildren().get(6))).getElements().size()>0) 
		{
			if(!c.getColor().equals(Color.rgb(0,255 , 0).toString())) {
				try {
					a.start11(stage, s1, s2,game,c);
		...
		else if(((Path)Shape.intersect(c.ball, (Shape) this.getG().getChildren().get(7))).getElements().size()>0) 
		{
			if(!c.getColor().equals(Color.rgb(255,255 , 0).toString())) {
				try {
					a.start11(stage, s1, s2,game,c);
